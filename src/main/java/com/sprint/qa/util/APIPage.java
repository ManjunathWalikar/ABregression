...
public class APIPage extends TestBase {
    ...
    public static String qa3_getoptionsWithToken() throws IOException {
        RestAssured.baseURI = prop.getProperty("apiurl");
        RequestSpecification request = RestAssured.given();
...
//    }
    ...
    public static int getVICreatedAPISubIDandPOUnique() throws IOException {
        ...
        //request.header("Content-Type", "application/json");
        request.header(new Header("Authorization", " Bearer " + qa3_getoptionsWithToken()));
...
//        return responsefromspecification.prettyPrint();
    ...
    public static Response preOrderValidationCRPostRequest(String url, String token){
        RequestSpecification request = RestAssured.given();
        String payload = "e1b4915a-4e7d-4182-9404-a3fb60b2c512";
        request.header("Content-Type", "application/json");
        request.header(new Header("Authorization", "Bearer" + token));
        Response response = request.body(payload).post(url);
